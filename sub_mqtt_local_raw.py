...
parser.add_option(
    "-d", "--display-lcd", action="store_true", help="print message to raspberry LCD"
)
parser.add_option(
    "-l", "--long-detail", action="store_true", help="print detail JSON message"
)
parser.add_option(
    "-t",
    "--topic",
...
def on_message(client, userdata, msg):
    ...
    if msg.topic[:11] == "GIOT-GW/UL/":
        print(
            ...
            + str(sensor_mac)[8:]
            + "\tCount:"
            + str(sensor_count).rjust(6)
            + "\tSNR:"
            + str(sensor_snr).rjust(4)
            + "\tRSSI:"
            + str(sensor_rssi).rjust(4)
            + "\tGWID:"
            + str(gwid_data).rjust(8)
        )
        ...
        print(
            ...
            + msg.topic[18:]
            + "\tStatus:"
            + str(json.loads(json_data)["status"])
            + "\tID:"
    ...
    if msg.topic[:7] == "GIOT-GW" and msg.topic[:17] != "GIOT-GW/DL-report":
        try:
            print(bytearray.fromhex(sensor_data).decode(), str(sensor_mac)[8:].upper())
            if (
                bytearray.fromhex(sensor_data).decode() == str(sensor_mac)[8:].upper()
                and options.downlink
            ):
                print(
                    "\x1b[6;30;42m"
                    + "pub_dl_local.py -i "
            ...
            print(
                ...
                + "HEX2ASCII:"
                + "\x1b[0m"
                + bytearray.fromhex(sensor_data).decode()
            )
